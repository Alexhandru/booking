    ...
    $('#fetch').submit(function(e){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                $('#fetchResults').append('We found ' + resultCount + ' results matching your criteria <br><br>');
                $.each(result, function(key, value){
                    //console.log(value.City);
                    //$('#fetchResults').append(value.Name + ' ' + value.Adress + ' ' + value.City + '<br>')
                    $('#fetchResults').append('<div class="card" style="width: 18rem;">' +
                    ...
                    '</ul>' +
                    '<div class="card-body">' +
                      '<a href="#" class="card-link">Go to rooms</a>' +
                    '</div>' +
                  '</div>' +
                  '<br>')
                });
